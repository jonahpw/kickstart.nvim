...
function _G.custom_tabline()
  ...
  for i = 1, vim.fn.tabpagenr '$' do
    ...
    if ok and diff_label then
      ...
      label = vim.fn.fnamemodify(name, ':p:h:t') .. '/' .. vim.fn.fnamemodify(name, ':t')
    end
    if i > 1 then s = s .. '%#TabLine#│' end
...
-- Auto-reload buffers when files change on disk (e.g. after git checkout)
vim.o.autoread = true
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          vim.keymap.set('n', 'grD', function() builtin.lsp_definitions { jump_type = 'vsplit' } end, { buffer = buf, desc = '[G]oto [D]efinition in vsplit' })

          vim.keymap.set('n', 'grT', function() builtin.lsp_definitions { jump_type = 'tab' } end, { buffer = buf, desc = '[G]oto definition in new [T]ab' })
